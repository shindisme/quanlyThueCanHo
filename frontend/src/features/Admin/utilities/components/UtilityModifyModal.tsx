...
export default function UtilityModifyModal({
  ...
  const currentApartment = useMemo(
    () => apartments.find((a) => a.id === editItem?.apartment_id),
    [apartments, editItem]
  );

  const currentBuilding = useMemo(
    () => (currentApartment ? buildings.find((b) => b.id === currentApartment.building_id) : null),
    [buildings, currentApartment]
  );

  const isSubmitDisabled = saving || !electricNew || !waterNew;
    ...
    <Modal
      ...
      footer={
        <>
          ...
          {!isViewOnly && (
            <Button onClick={handleUpdateUtilityReading} isLoading={saving} disabled={isSubmitDisabled}>
              Lưu thông tin
            </Button>
          )}
        </>
