...
export default function GuestApartmentListing() {
  ...
  const filtered = mockApartments.filter((a) => {
    ...
    return matchSearch && matchStatus && matchBuilding && matchPrice;
  });
              ...
              >
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between mb-2">
                    <h3 className="font-semibold text-gray-800">{apt.apartment_code} - {apt.title}</h3>
                    <Badge variant={APARTMENT_STATUS_COLORS[apt.status] as "success" | "info" | "warning"}>
                      {APARTMENT_STATUS_LABELS[apt.status]}
                    </Badge>
                  </div>
                  <p className="text-xs text-gray-400">{building?.name} - {building?.address}</p>
