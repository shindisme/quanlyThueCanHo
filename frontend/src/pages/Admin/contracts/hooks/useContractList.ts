...
export function useContractList() {
  ...
  const displayContracts = (() => {
    if (role === "MANAGER" && managedBuildingId) {
      const buildingApartmentIds = apartments
        ...
        .map((a) => a.id);
      return contracts.filter((c) => c.apartment?.building_id === managedBuildingId || buildingApartmentIds.includes(c.apartment_id));
    }
    if (role === "TENANT") {
  ...
  const { items: sortedContracts, requestSort, getSortIcon } = useSort(
    ...
    {
      id: (item) => item.id,
      tenant: (item) => item.tenant?.full_name || tenants.find((t) => t.id === item.tenant_id)?.full_name || "",
      apartment: (item) => {
