...
export function useApartmentDetail() {
  ...
  const { data: contracts = [], isLoading: loadingContracts } = useQuery({
    queryKey: ["contracts", "apartment", id],
    queryFn: () => contractService.getAllContractPages({ apartmentId: Number(id) }).catch(() => []),
  });

  const { data: tenantsRes, isLoading: loadingTenants } = useQuery({
    queryKey: ["tenants"],
    queryFn: () => tenantService.getAllTenantPages().catch(() => []),
  });
  ...
  const apartmentContracts = apartment?.contracts || [];
  const activeContract = apartmentContracts.find((contract) => contract.status === "ACTIVE")
