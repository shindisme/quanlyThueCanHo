  ...
  const columns: Column<ApartmentData>[] = [
    {
      ...
      render: (apt) => {
        ...
        return (
          <div
            className="font-semibold text-primary-650 cursor-pointer hover:underline"
    ...
    },
    {
      ...
      render: (apt) => <span className="text-gray-600">{apt.area} m²</span>
    },
    {
      key: "bedrooms",
      label: "Phòng ngủ",
      sortValue: (apt) => apt.bedrooms,
      render: (apt) => <span className="text-gray-655">{apt.bedrooms}</span>
    },
    {
      key: "bathrooms",
      label: "Phòng vệ sinh",
      sortValue: (apt) => apt.bathrooms,
      render: (apt) => <span className="text-gray-655">{apt.bathrooms}</span>
    },
    {
      key: "rental_price",
      label: "Giá thuê",
      sortValue: (apt) => apt.rental_price,
      render: (apt) => <span className="font-semibold text-gray-855">{formatCurrency(apt.rental_price)}</span>
    },
    {
      key: "status",
      label: "Trạng thái",
          ...
          {role !== "STAFF" && (
            <>
              ...
              <button
                ...
                className="p-2 rounded-lg text-gray-400 hover:text-red-650 hover:bg-red-55/20 cursor-pointer"
                title="Xóa"
              >
                <Trash2 size={16} />
              </button>
