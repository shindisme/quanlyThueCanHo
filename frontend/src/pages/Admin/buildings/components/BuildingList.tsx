...
export default function BuildingList({
  ...
  const columns: Column<BuildingData>[] = [
    {
      key: "branch_name",
      label: "Tên chi nhánh",
      ...
      render: (b) => (
        <div className="flex items-center justify-end gap-1">
          ...
          {role === "ADMIN" && (
            <>
              <button
                onClick={() => { setEditItem(b); setShowModifyModal(true); }}
                className="p-2 rounded-lg text-gray-400 hover:text-primary-600 hover:bg-primary-50 cursor-pointer"
                title="Sửa"
              >
                <Pencil size={16} />
  ...
  return (
    <div className="mt-6">
      <DataTable columns={columns} data={sortedBuildings} emptyMessage="Không tìm thấy chi nhánh nào." />
    </div>
  );
}
